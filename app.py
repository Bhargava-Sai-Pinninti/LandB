...
genai.configure(api_key='AIzaSyD1BVDoduCIEDTi0fFe6ZvzRUIfB26v3Xo')
...
def generate_recommendations(appliances):
    ...
    prompt += "Based on the load, what kVA line,  stabilizer , inverter and the battery capacity [ inverter runtime for 5 hours , 12 hours , 24 hours  ] , would you recommend? give response easily understandable with numbering and , dont show the load calculation part in the response but show the total load value in response . and some product recommendations which are considarable to buy . the totaal response is in text only dont include any tables"
...
def generate_prompt_billestimation(appliances, maxbudget, ratePerKWh, unitsConsumed, billEstimation):
    ...
    bill_str = f"Per Day: ₹{billEstimation['perDay']}, Per Month: ₹{billEstimation['perMonth']}, Per Year: ₹{billEstimation['perYear']}"
    ...
    prompt = (f"I was given the following home appliances and their load details. "
              ...
              f"Based on the load and my monthly budget, suggest how to use the appliances for better utilization and to ensure the cost of the bill is under my maximum budget per month. and any best suggestions by you can give to me")
